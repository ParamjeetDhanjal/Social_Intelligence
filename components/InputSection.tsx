...
import { Loader2, Link as LinkIcon, ImageIcon, FileCheck } from 'lucide-react';
...
const InputSection: React.FC<InputSectionProps> = ({ 
  ...
  const totalPages = Math.ceil(siteRows.length / itemsPerPage);
                        ...
                        <td className="border-b border-r border-slate-700 p-0 relative min-w-[90px]">
                            <div className="flex w-full h-full items-center justify-center px-2 py-1">
                               {row.auditStatus === 'GO' ? (
                                 ...
                                 <button 
                                   onClick={() => handleAuditSite(globalIdx)} 
          ...
          <div className="flex justify-center items-center gap-2 mt-4 pb-2">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(num => {
              ...
              return (
                ...
                >
                  <span className="text-[9px] text-slate-500 font-normal mr-1">{startIdx + 1}–{Math.min(startIdx + itemsPerPage, siteRows.length)}</span>
                  {num}
                </button>
              );
            })}
